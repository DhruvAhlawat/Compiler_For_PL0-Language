...
functor makeRational (bi : BIGINT): RATIONAL =
    ...
    fun equal((a,b):rational, (c,d):rational) = if(a = c andalso d = b) then true else false; 
    ...
    let
        ...
        val (N,a) = parseTill(a,#"("); (*if this type of decimal is in the form without a (.) then this will be the entire decimal form*)
        val (R,a) = parseTill(a,#")"); (*now all of S,I,N,R have been parsed*)
    in
        if(R = []) then (sign,I,N,[#"0"])  else (sign,I,N,R)
    end;
